...
class WPForms_DataLayer_Integration {
    ...
    public function handle_form_submit($fields, $entry, $form_data, $entry_id) {
        ...
        $exclude_ids = array();
        // Sostituiamo !empty con un controllo più rigido per salvare l'ID '0'
        if (isset($form_data['settings']['datalayer_exclude_ids']) && trim($form_data['settings']['datalayer_exclude_ids']) !== '') {
...
if (file_exists($puc_path)) {
    ...
    $myUpdateChecker = \YahnisElsts\PluginUpdateChecker\v5\PucFactory::buildUpdateChecker(
        'https://github.com/CreativeMetrics/wpforms-datalayer', // <-- Ricordati di ripristinare la tua repository qui!
        __FILE__,
        'wpforms-datalayer-integration'
    );
